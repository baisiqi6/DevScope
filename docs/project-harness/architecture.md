...
3D 渲染沿用 Three.js / `react-force-graph-3d`：仓库球体使用程序化表面颜色与粗糙度，技术栈
使用受光的八面体，环境反射仅在挂载时生成一次。普通节点和文字不做全场景 Bloom，画布与后处理缓冲区统一限制为最多 1.5 倍像素比。
初始取景优先包含至少两条连接的主图节点（不足三个时取全图），孤立节点仍保留在场景中，可缩放查看。
语言黑洞由可遮挡背景几何体的黑色核心、面向相机的观测图像组成；固定轨道平面的投影与
近似后方光弧随相机角度变化，光子环不随平面吸积盘退化成侧面的细线。
`apps/web/src/lib/graph-black-hole.ts` 维护对应 shader 与透镜筛选。

所有可见黑洞保留轮廓，近处增加盘面纹理；背景扭曲共用一次场景采样 pass，每帧最多
处理 8 个投影半径至少 2 CSS px 的黑洞，优先焦点及较大投影，离屏剔除，重叠区域只取
最强透镜。半径随世界尺寸和相机距离变化，不使用固定像素圈。搜索定位同时设置选中状态。
这是实时视觉近似，不求解相对论光线轨迹；屏幕空间扭曲不区分前景/背景深度，不重建
画面外或被遮挡的信息。减少动态效果时页面回退 2D；3D 组件自身也冻结盘面时间并关闭透镜。
