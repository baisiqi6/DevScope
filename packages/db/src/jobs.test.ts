...
describe("持久任务队列", () => {
  ...
  it("technology stack backfill 使用全局 active singleton 并返回现有任务", async () => {
    const existing = createJob({
      id: 44,
      userId: 9,
    ...
    await expect(enqueueTechnologyStackEntitiesBackfillJob(db as any, {
      userId: 7,
      version: "v2",
      requestedAt: new Date("2026-08-18T00:00:00Z"),
    })).resolves.toEqual({ job: existing, enqueued: false });
  ...
  it("technology stack backfill 跨用户拒绝复用终态 version", async () => {
    const terminal = createJob({
      id: 45,
      userId: 9,
      type: "technology_stack.entities.backfill",
      idempotencyKey: "technology-stack:entities:backfill:v1",
      status: "succeeded",
      result: { outcome: "applied", version: "v1", processed: 40, conflicts: [] },
    });
    const insertReturning = vi.fn().mockResolvedValue([]);
    const activeLimit = vi.fn().mockResolvedValue([]);
    const terminalLimit = vi.fn().mockResolvedValue([terminal]);
