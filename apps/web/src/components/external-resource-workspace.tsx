...
export function ResourceCard({ resource, groups, onAddToGroup, onEdit, onDelete, onToggleRead, onTogglePinned, density, pending }: ResourceCardProps) {
...
export function ExternalResourceWorkspace() {
  ...
  return (
    <main className="min-h-screen"><AnimatedBackground /><div className="container mx-auto max-w-7xl px-4 py-6 sm:py-8">
      <header className="command-page-header mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="command-kicker">研究资料库</p><h1 className="text-2xl font-semibold tracking-tight">外部资源</h1><p className="mt-1 max-w-2xl text-sm text-muted-foreground">把文章、论文和工具网站收进独立的预览收藏库。当前只保存预览元数据，不抓取正文。</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => resourcesQuery.refetch()} disabled={resourcesQuery.isFetching}><RefreshCw className={cn(resourcesQuery.isFetching && "animate-spin")} />刷新</Button><Button onClick={() => setShowCreate(true)}><Plus />保存资源</Button></div></header>
