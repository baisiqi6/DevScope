...
interface ResourceCardProps {
  ...
  onEnableContent?: (resource: ExternalResource) => void;
  onReadContent?: (resource: ExternalResource) => void;
  density: "grid" | "list";
  pending: boolean;
}

export function ResourceCard({ resource, groups, onAddToGroup, onEdit, onDelete, onToggleRead, onTogglePinned, onEnableContent, onRequestContent, onReadContent, density, pending }: ResourceCardProps) {
  ...
  return (
    <Card className={cn("group flex h-full flex-col overflow-hidden transition-[transform,border-color,background-color] duration-150 hover:-translate-y-0.5 hover:border-border-hover hover:bg-card-hover", density === "list" && "lg:grid lg:grid-cols-[minmax(180px,260px)_1fr]", resource.isPinned && "border-primary/35")}>
      ...
      <CardFooter className="flex flex-wrap gap-2 border-t border-border/60 p-3">
        ...
        {resource.ingestionMode === "preview_only" && onEnableContent && <Button size="sm" variant="outline" onClick={() => onEnableContent(resource)} disabled={pending}>启用正文采集</Button>}
...
export function ExternalResourceWorkspace() {
  ...
  const enableContent = trpc.externalResources.enableContent.useMutation({ onSuccess: () => resourcesQuery.refetch() });
  ...
  const enableResourceContent = (resource: ExternalResource) => enableContent.mutate({ resourceId: resource.id });
