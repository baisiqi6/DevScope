...
console.log("2. After loading .env:");
console.log("   GITHUB_TOKEN:", process.env.GITHUB_TOKEN ? "SET" : "NOT SET");
console.log("   DATABASE_URL:", process.env.DATABASE_URL ? "SET" : "NOT SET");
console.log("   BGE_API_URL:", process.env.BGE_API_URL || "NOT SET");
console.log("   SILICONFLOW_API_KEY:", process.env.SILICONFLOW_API_KEY ? "SET" : "NOT SET");
...
console.log("3. Testing BGEEmbeddingProvider initialization:");
try {
  const { BGEEmbeddingProvider } = require("./packages/ai/dist/index.mjs");
...
try {
  const { createDb } = require("./packages/db/dist/index.mjs");
  console.log("   Module loaded successfully");
...
// 4. 测试 Pipeline 创建
console.log("5. Testing Pipeline creation:");
try {
  const { createDb, createPipeline } = require("./packages/db/dist/index.mjs");
  const db = createDb();
